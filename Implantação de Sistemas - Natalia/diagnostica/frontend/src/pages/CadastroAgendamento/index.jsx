...
export default function CadastroAgendamento() {
  ...
  const carregarAgendamentos = async (termo = '') => {
    try {
      const res = await axios.get(`${API}/agendamentos`, { params: { busca: termo } });
  ...
  const handleSalvar = async (e) => {
    ...
    if (!form.id_cliente || !form.id_profissional || !form.data_agendamento || !form.hora_agendamento) {
    ...
    try {
      ...
      setErroForm(err.response?.data?.erro || 'Erro ao salvar agendamento.');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-agenda-bg p-6">
      <button
        ...
        className="text-agenda-blue hover:underline mb-6 flex items-center gap-1"
      >
        ← Voltar
      </button>
      ...
      <div className="bg-white rounded-lg shadow-md overflow-x-auto">
        <table className="w-full text-sm text-left">
          ...
          <tbody>
            ...
            {agendamentos.map((ag) => (
              <tr key={ag.id_agendamento} className="border-t border-gray-100">
                <td className="px-4 py-3">{ag.cliente_nome}</td>
                <td className="px-4 py-3">{ag.profissional_nome}</td>
                <td className="px-4 py-3 capitalize">{ag.tipo_servico}</td>
                <td className="px-4 py-3">{new Date(ag.data_agendamento).toLocaleDateString('pt-BR')}</td>
                <td className="px-4 py-3">{ag.hora_agendamento}</td>
                <td className="px-4 py-3 capitalize">{ag.status}</td>
                <td className="px-4 py-3 flex gap-3">
                  ...
                  <button
                    onClick={() => handleExcluir(ag.id_agendamento)}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50">
          ...
          >
            ...
            <select
              ...
              onChange={(e) => setForm({ ...form, id_profissional: e.target.value })}
              className="w-full border border-gray-300 rounded-md px-3 py-2 mb-3"
            >
              <option value="">Selecione...</option>
              {profissionais.map(p => (
            ...
            {erroForm && <p className="text-red-500 text-sm mb-3">{erroForm}</p>}
